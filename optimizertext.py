...
class SimpleDashScopeEngine:
    ...
    def __call__(self, user_text: str, system_prompt: str = None) -> str:
        return self.generate(user_text=user_text, system_prompt=system_prompt)
...
def optimize_h2_with_textgrad(
    ...
    h2_var = tg.Variable(
        f"{old_h2_val:.6f}",
        requires_grad=True,
        role_description="The numeric value (as text) for height_differences[1] (second segment)."
    )
    ...
    def eval_fn_height():
        try:
            ...
            loss_val = 1e12
            return tg.Variable(str(loss_val), requires_grad=True, role_description="penalize non-numeric h2")
        ...
        return tg.Variable(
            ...
            requires_grad=True,
            role_description="Total track cost to minimize by adjusting h2."
        )

    optimizer = tg.TextualGradientDescent(engine=backward_engine, parameters=[h2_var])
...
def main():
    ...
    backward_engine = SimpleDashScopeEngine(forward_client, forward_model_name)
    ...
    if mode == "helper" or "--helper" in sys.argv:
        ...
        if not use_llm:
            ...
            if old_h2 is not None and new_h2_neighbor is not None:
                ...
                new_h2 = round(new_h2_neighbor + step_guess, 3)
                print(
                    f"[Coop 2->{target_id} | Iter {coop_iter}] new_h2={new_h2} step=0.5 note=rule-fallback")
            else:
